...
router.post('/books', isAdmin, upload.single('cover'), validate(bookSchema), asyncErrorHandler(bookController.create_book));
router.put('/books/:id', isAdmin, upload.single('cover'), validate(bookSchema), asyncErrorHandler(bookController.update_book));
